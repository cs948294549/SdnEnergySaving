...
public class SwitchForwarding extends ForwardingBase implements IFloodlightModule {

	public static boolean canForwarding = true;
	protected IRestApiService restApi;

	protected static Logger log = LoggerFactory.getLogger(SwitchForwarding.class);

	private UDPService udpService = new UDPService();
	...
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision,
		...
		if (!canForwarding) {
			...
			if (eth.getPayload() instanceof IPv4) {
				...
				log.info("Forwarding模块发送数据：" + src + "--" + dst);
				udpService.send(src + "--" + dst);
			} else {
				log.info("Forwarding模块路由查找被禁止");
			}
			...
			while ((iSrcDaps < srcDaps.length) && (iDstDaps < dstDaps.length)) {
				...
				if (srcVsDest == 0) {
					if (!srcDap.equals(dstDap)) {
						Route route = routingEngine.getRoute(srcDap.getSwitchDPID(), (short) srcDap.getPort(),
								dstDap.getSwitchDPID(), (short) dstDap.getPort(), 0); // cookie
																						// =
																						// 0,
																						// i.e.,
																						// default
						...
						if (route != null) {
							...
							pushRoute(route, match, wildcard_hints, pi, sw.getId(), cookie, cntx,
									requestFlowRemovedNotifn, false, OFFlowMod.OFPFC_ADD);
						}
					}
					iSrcDaps++;
					iDstDaps++;
	 ...
	 */
	@LogMessageDoc(level = "ERROR", message = "Failure writing PacketOut " + "switch={switch} packet-in={packet-in} "
	...
	protected void doFlood(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		try {
			if (log.isTraceEnabled()) {
				log.trace("Writing flood PacketOut switch={} packet-in={} packet-out={}", new Object[] { sw, pi, po });
			}
			messageDamper.write(sw, po, cntx);
		} catch (IOException e) {
			log.error("Failure writing PacketOut switch={} packet-in={} packet-out={}", new Object[] { sw, pi, po }, e);
		}
